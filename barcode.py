...
if len(contours) !=0:
    ...
    # tim contour lon nhat
        ...
        filter_img=cv2.blur(masked_img,(7,7))
        # cv2.imshow('max1',filter_img)
        thresh, filter_img2 = cv2.threshold(filter_img, 150 , 255, cv2.THRESH_OTSU)
        cv2.imshow('max2',filter_img2)

        filter_img3=cv2.blur(filter_img2,(7,7))
        # cv2.imshow('max1',filter_img)
        thresh, filter_img3 = cv2.threshold(filter_img2, 150 , 255, cv2.THRESH_OTSU)
        filter_img=cv2.blur(filter_img3,(7,7))
        # cv2.imshow('max1',filter_img)
        thresh, filter_img4 = cv2.threshold(filter_img3, 150 , 255, cv2.THRESH_OTSU)
        filter_img=cv2.blur(filter_img4,(7,7))
        thresh, filter_img3 = cv2.threshold(filter_img4, 150 , 255, cv2.THRESH_OTSU)
        # cv2.imshow('max4',filter_img4)
        filter_img5=cv2.blur(filter_img4,(7,7))
        # cv2.imshow('max5',filter_img5)
        thresh, filter_img6 = cv2.threshold(filter_img5, 150 , 255, cv2.THRESH_OTSU)
        cv2.imshow('max6',filter_img6)
        kernel=np.ones((5,5),np.uint8)
        erosion= cv2.dilate(filter_img2,kernel,iterations=1)
        kernel=np.ones((5,5),np.uint8)
        erosion= cv2.dilate(filter_img3,kernel,iterations=1)
        kernel=np.ones((5,5),np.uint8)
        erosion= cv2.dilate(filter_img4,kernel,iterations=1)
        kernel=np.ones((5,5),np.uint8)
        erosion= cv2.dilate(filter_img5,kernel,iterations=1)
        cv2.imshow('max5',filter_img5)
        masked_img_final=cv2.bitwise_and(img5,img5,mask=erosion)
        x=100
        y=100
        cv2.putText(masked_img_final,'nnnn',(x,y),font,0.5,(255,0,0))
        # cv2.imshow('max contour',masked_img_final)
        t_lower = 50
        t_upper = 150
        img_canny = cv2.Canny(filter_img5,t_lower,t_upper,L2gradient=True)
